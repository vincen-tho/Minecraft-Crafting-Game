...
// Resep
private:
  const int *dimension; // row x col
  const string **input;
  const string output; // Nama item output
  const int output_q;  // Jumlah item output
public:
  // Bikin Resep
  //
  // Input :
  ...
  //  output    : string
  //  output_q  : int
  Recipe(int *dimension, string **input, string output, int output_q);
  ~Recipe();
...
// List Resep
private:
  Recipe recipes[RECIPE_MAX_SIZE];
  int size;
